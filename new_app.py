...
class App:
    def __init__(self, window, window_title, video_source=0):
        ...
        for val in values:
            tk.Radiobutton(window, 
                ...
                pady = 20, 
                variable=self.v,
                value=val).pack(side=tk.RIGHT)
            # self.radio1.grid(row = 0, column = 0)

        # Create a canvas that can fit the above video source size
        self.canvas = tk.Canvas(window, width = w * 0.6, height = h * 0.9, bg=BGCOLOR)
        ...
        self.btn_start=tk.Button(start_cancel, bd=0, bg='#98e6e6', text='START', command=self.open_camera, height = 5, width = 10)
        self.btn_start.pack(side=tk.LEFT, padx=10,)

        self.btn_stop=tk.Button(start_cancel,bd=0, bg='#98e6e6', text='STOP', command=self.close_camera, height = 5, width = 10)
        self.btn_stop.pack(side=tk.LEFT, padx=10,)

        # quit button
        self.btn_quit=tk.Button(start_cancel,bd=0, bg='#98e6e6', text='QUIT', command=sys.exit, height = 5, width = 10)
    ...
    def update(self):
        ...
        if self.ok:
            ret, frame = self.videoCap.get_frame(self.v.get())
...
class VideoCapture:
    def __init__(self, video_source=0):
        ...
        self.score = False
        self.img_size = 128
        self.model = load_model(os.path.join(THIS_FOLDER, 'cnnBasic.h5'))
        self.faceCascade = cv2.CascadeClassifier(os.path.join(THIS_FOLDER, 'haarcascade_frontalface_default.xml'))
        self.eye_cascade_main = cv2.CascadeClassifier(os.path.join(THIS_FOLDER, 'haarcascade_eye.xml'))
        self.eye_cascade_backup = cv2.CascadeClassifier(os.path.join(THIS_FOLDER, 'haarcascade_righteye_2splits.xml'))
